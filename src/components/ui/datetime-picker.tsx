...
export function DateTimePicker({
  ...
  const dislayFormat = useMemo(() => {
    if (!displayValue) return 'Виберіть дату'
    return format(
      displayValue,
      `d MMMM yyyy - ${!hideTime ? (use12HourFormat ? ' hh:mm:ss a' : ' HH:mm:ss') : ''}`,
    ...
    <Popover open={open} onOpenChange={setOpen} modal={modal}>
      <PopoverTrigger asChild>
        {renderTrigger ? (
          ...
          <div
            className={cn(
              'flex bg-input w-full cursor-pointer items-center h-12 ps-3 pe-1 font-normal border border-border rounded-xl text-sm',
      ...
      <PopoverContent className="w-auto p-2">
          ...
          <div className="text-md font-bold ms-2 flex items-center cursor-pointer">
            <div>
              <span onClick={() => setMonthYearPicker(monthYearPicker === 'month' ? false : 'month')}>
                {format(month, 'MMMM', { locale: uk })}
              </span>
              <span className="ms-1" onClick={() => setMonthYearPicker(monthYearPicker === 'year' ? false : 'year')}>
                {format(month, 'yyyy', { locale: uk })}
              </span>
            </div>
            <Button variant="ghost" size="sm" onClick={() => setMonthYearPicker(monthYearPicker ? false : 'year')}>
        ...
        <div className="flex flex-col gap-2">
          ...
          <div className="flex flex-row-reverse items-center justify-between">
            <Button className="ms-2 h-10 px-8" onClick={onSubmit}>
              Зберегти
            </Button>
            {timezone && (
...
function MonthYearPicker({
  ...
  const months = useMemo(() => {
    ...
    for (let i = 0; i < 12; i++) {
      ...
      months.push({ value: i, label: format(new Date(0, i), 'MMM', { locale: uk }), disabled })
    }
    return months
  }, [value])
...
function TimePicker({
  ...
  const [ampm, setAmpm] = useState(format(value, 'a', { locale: uk }) === 'AM' ? AM_VALUE : PM_VALUE)
  const [hour, setHour] = useState(use12HourFormat ? +format(value, 'hh', { locale: uk }) : value.getHours())
  ...
  const display = useMemo(() => {
    ...
    return format(value, arr.join(':') + (use12HourFormat ? ' a' : ''), { locale: uk })
  }, [value, use12HourFormat, timePicker])

  return (
    <Popover open={open} onOpenChange={setOpen}>
      <PopoverTrigger asChild>
        <Button variant="primary" role="combobox" aria-expanded={open} className="justify-between mt-4">
...
function buildTime(options: BuildTimeOptions) {
  ...
  let date: Date
  if (use12HourFormat) {
    const dateStrRaw = format(value, formatStr, { locale: uk })
