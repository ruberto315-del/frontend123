import { useCreateCourses, useOneCourses, useUpdateCourses } from '@/api/hooks/use-courses'
...
const formSchema = z.object({
  ...
  status: z.enum(["DRAFT", "PLANNED", "ARCHIVED"], "Статус заходу обов'язковий"),
  registrationOpen: z.enum(["OPENED", "CLOSED"], "Статус реєстрації обов'язковий"),
...
const AdminFullCoursePage = () => {
    ...
    const {data: course, isLoading: isCoursesLoading} = useOneCourses(isUpdate ? params?.id : undefined)
  ...
  const formData = {
    ...initialFormData,
    ...(course ? { ...course, registrationOpen: course.registrationOpen ? 'OPENED' : 'CLOSED' } : {}),
    ...userFormData,
  ...
  if (isUpdate) {
      ...
      createCourse.mutate({ ...formData, registrationOpen: formData.registrationOpen === 'OPENED' ? true : false })
    }
    
  }

const isLoading = isCoursesLoading || createCourse.isPending || updateCourse.isPending
...
const changeUserFormData = (key: keyof FormData, value: string) => {
  ...
  return (
    <div className="container mx-auto px-4 py-12 md:py-16 max-w-4xl">
      <Title className="mb-12">{isUpdate? "Овновити захід" : "Новий захід"}</Title>
      <Card>
        <CardContent>
            <BaseField className='space-y-6'
            name="name"  
            required
            value={formData.name}
            type="text"
            ...
            <div className='grid grid-cols-2 md:grid-cols gap-4'>
                ...
                <div>
                 <label className="block text-sm font-medium text-text-primary mb-2">Програма заходу</label>
            ...
            value=""
            type="select"
            onChange={(value) => changeUserFormData("certificateTemplateId", value)}
            label = "Шаблон сертифікату"
            placeholder='500 грн.'
            items={[
                {label: "Шаблон №1", value: "1"}
            ]}
            />
        ...
        {showErrors && !!getFormErrors(errors).length &&(
            ...
            <div className='flex gap-4 mt-10'>
                <Button className='flex-1' size="lg" disabled={isLoading} onClick={handleSubmit} >{isUpdate ? "Оновнити" : "Зберегти"}</Button>
                <Button className='w-50' variant="ghost" size="lg" disabled={isLoading}>Відмінити</Button>
            </div>
        </CardContent>
        </Card>
    </div>
